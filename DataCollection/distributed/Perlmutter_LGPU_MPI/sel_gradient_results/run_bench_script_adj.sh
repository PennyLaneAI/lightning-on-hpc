...
num_qubitsL=38 # Set the largest number of qubits outside the loop
num_qubitsS=33 # Set the smallest number of qubits outside the loop

num_qubits=$((num_qubitsL))
num_runs=1

#run the largest task that make use of all nodes first
LOG_FILE="output_sel_adj_${num_nodes}_nodes_${n}_gpus_${num_qubits}_qubits.log"
srun --nodes=$num_nodes --ntasks-per-node=4 -n $n python test_sel_adj.py $num_qubits > "$LOG_FILE" 2>&1
...
for i in $(seq $((num_qubitsL - num_qubitsS)) -1 1); do
	...
	srun --nodes=$num_nodes --ntasks-per-node=4 -n $n python test_sel_adj.py $num_qubits > "$LOG_FILE" 2>&1 &
    	...
    	srun --nodes=1 -n $n python test_sel_adj.py $num_qubits > "$LOG_FILE" 2>&1 &
    fi
