...
devicename = "lightning.kokkos"

ompnum = os.environ.get("OMP_NUM_THREADS", "0")
for mol, basis_set in zip(mols, basis_sets):
    ...
    wires = ham.wires
    dev = qml.device(devicename, wires=wires)
    ...
    max_iterations = 10 if len(wires) <= 20 else 1
    conv_tol = 1e-16
    timetot = 0
    ...
    message = f"{mol}, {basis_set}, {len(wires)}, {len(ham.coeffs)}, {timetot / max_iterations * 10}"
    print(message)
    with open(f"{devicename}-{ompnum}-data.txt", "a") as f:
        f.write(message+"\n")
